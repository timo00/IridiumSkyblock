            ...
            if (!user.getIsland().equals(u.getIsland()) && u.getIsland() != null) {
                if (user.bypassing || user.getIsland().getPermissions(user.getRole()).coop) {
                    if (user.getIsland().coopInvites.contains(u.islandID)) {
                        ...
                        sender.sendMessage(Utils.color(IridiumSkyblock.getMessages().coopInviteSent.replace("%player%", UserManager.getUser(u.getIsland().getOwner()).name).replace("%prefix%", IridiumSkyblock.getConfiguration().prefix)));
                    }
                } else {
